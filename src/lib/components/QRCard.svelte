...
<article class="card flex flex-col gap-4 transition-colors hover:border-border-strong">
  ...
  <div class="grid grid-cols-5 gap-1.5 pt-1">
    <button on:click={copyUrl} class="btn-ghost btn-sm" aria-label={copied ? 'URL copied' : 'Copy URL'} title="Copy short URL" aria-live="polite">
      {#if copied}
    ...
    <a href={`/dashboard/qr/${qr.short_code}`} class="btn-ghost btn-sm" aria-label="Edit" title="Edit settings">
    ...
    <a href={`/dashboard/qr/${qr.short_code}/stats`} class="btn-ghost btn-sm" aria-label="Stats" title="View scan stats">
    ...
    <button on:click={() => onToggle(qr.short_code, !qr.is_active)} class="btn-ghost btn-sm" aria-label={qr.is_active ? 'Disable' : 'Enable'} title={qr.is_active ? 'Disable code' : 'Enable code'}>
    ...
    <button on:click={() => onDelete(qr.short_code)} class="btn-ghost btn-sm text-danger hover:bg-danger/10 hover:text-danger" aria-label="Delete" title="Delete code">
