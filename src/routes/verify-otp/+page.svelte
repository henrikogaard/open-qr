...
<svelte:head>
  <title>Verify code — Open-QR</title>
</svelte:head>
  ...
  <div class="w-full max-w-md">
    ...
    <div class="card p-8">
        ...
        <form on:submit|preventDefault={verify} class="space-y-5">
          <div>
            ...
            <input
              ...
              maxlength="6"
              required
              class="input ps-[0.5em] text-center text-2xl font-mono tracking-[0.5em]"
            />
          </div>
      ...
      {:else}
        <div class="mb-6">
          <h1 class="text-2xl font-semibold text-fg">Verify code</h1>
          <p class="mt-1.5 text-sm text-fg-muted">No email was specified.</p>
        </div>
        <a href="/login" class="btn-primary w-full">Start again</a>
      {/if}
    </div>
  </div>
</div>
